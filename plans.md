...
# Exec Plan: モバイル向けShadcn UI適用

## 目的
主要操作をスマートフォンでも使いやすいカードレイアウトに再構成し、Shadcnデザインシステムに沿ったUIトークンを導入する。

## 実施項目
- [x] Shadcnベースのカラートークンとコンポーネントスタイルを `styles.css` に整備
- [x] `index.html` をカード/スタック構造へ再構築し、モバイルでも操作しやすい並びへ改善
- [x] UI更新に合わせてテキスト/警告/進捗表示のARIA設定を点検
- [ ] 手動動作確認（モバイル幅とデスクトップ幅でのレイアウト崩れがないこと）

## 注意/リスク
- TailwindなしでShadcnトークンを採用するため、CSS変数とコンポーネントの整合性を壊さないよう管理する。
- 既存のID/アクセシビリティ属性を維持し、スクリプトとの結合を壊さないよう慎重にDOMを更新する。

## To-Do
1. [x] 主要セクション（コントロール/ペースト/プレビュー）をカード化し、`controls` はモバイルで縦積み・タブレット以上で2列化する。
2. [x] ボタン群は折返し可能なグリッドに変更し、タッチ領域を確保する。
3. [x] 進捗・残時間をカードフッターにまとめ、読み上げ状態が変わってもジャンプしないよう余白を調整する。
