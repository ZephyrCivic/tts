...
# Exec Plan: モバイル向けUI刷新（Shadcn導入）

## 目的
主要操作を片手で扱えるモバイルフレンドリーなUIへ刷新し、Shadcn UIコンポーネントを採用して一貫したスタイルガイドを確立する。

## 実施項目
- [x] 仕様/UIの確定（Shadcnベースのレイアウト、モバイル優先の配置）
- [x] 実装（Vite + React + Tailwind/Shadcnセットアップ、主要コンポーネントの移植）
- [ ] 単体/統合テスト（UI操作のハンドラ確認、キュー連携）
- [ ] 受け入れ手順の更新（モバイル表示の検証観点を追記）

## 注意/リスク
- Shadcn導入によるバンドルサイズ増と依存管理。既存のビルドレス構成からの移行コスト。
- SpeechSynthesis API操作はReact化後も同等に動作させる必要がある。
- Tailwindのスタイル競合が既存CSSと重複する恐れがあるため整理する。

## To-Do
1. [x] Shadcnプリセットの導入方針（どのコンポーネントを使用するか）
2. [x] 既存CSSの段階的な削除/移行計画
3. [ ] モバイルでの操作テスト（画面幅375px前後）
